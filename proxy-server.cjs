...
const server = http.createServer((req, res) => {
  ...
  const probe = parsed.query.probe === 'true';
  const startTime = parseFloat(parsed.query.start || '0');
  ...
  // ── PROBE (devuelve duración y codec en JSON) ─────────────────
  if (probe) {
    ...
    let data = '';
    ffprobe.stdout.on('data', d => data += d);
    ffprobe.on('close', () => {
      let result = { duration: 0, codec: 'unknown' };
      try {
    ...
    ffprobe.on('error', () => {
      res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
      res.end(JSON.stringify({ duration: 0, codec: 'unknown' }));
    });
    return;
  }
  ...
  // ── TRANSCODE VOD (con seek por ?start=segundos) ──────────────
  if (transcode) {
    console.log(`→ Transcode: ${targetUrl} | start: ${startTime}s`);
    ...
    ffprobe.on('close', () => {
      ...
      // -ss ANTES de -i = fast seek por keyframe
  ...
  const makeRequest = (reqUrl, redirectCount = 0) => {
    ...
    catch (e) { res.writeHead(400); res.end(`Invalid URL: ${reqUrl}`); return; }
    ...
    const proxyReq = client.request(options, (proxyRes) => {
      if (proxyRes.statusCode === 301 || proxyRes.statusCode === 302 || proxyRes.statusCode === 307) {
        ...
        if (wasIpUrl && !isIpUrl) {
          ...
          const r2 = c2.request({
            hostname: p2.hostname, port: p2.port || 80,
            path: p2.pathname + p2.search, method: 'GET',
            headers: { 'User-Agent': 'Mozilla/5.0', 'Accept': '*/*' },
          }, rs2 => {
            res.writeHead(200, { 'Content-Type': rs2.headers['content-type'] || 'video/mp2t', 'Access-Control-Allow-Origin': '*' });
            rs2.pipe(res);
          });
      ...
      const isM3U8 = contentType.includes('mpegurl') || contentType.includes('x-mpegurl') ||
                     reqUrl.includes('.m3u8') || reqUrl.includes('.m3u');
      ...
      const responseHeaders = { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'Range', 'Content-Type': contentType || 'video/mp2t' };
